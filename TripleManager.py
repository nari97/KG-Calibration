...
class TripleManager():
    ...
    def __init__(self, path, splits, nbatches=None, neg_ent=None, neg_rel=None, use_bern=False, seed=None, corruption_mode="Global"):
        ...
        self.headDict, self.tailDict, self.dom, self.ran = {}, {}, {}, {}
        ...
        for s in splits:
            ...
            for r in loader.relations:
                ...
                if r in loader.getDomain():
                    if r not in self.dom:
                        self.dom[r] = set()
                    self.dom[r] = self.dom[r].union(loader.getDomain()[r])

                if r in loader.getRange():
                    if r not in self.ran:
                        self.ran[r] = set()
                    self.ran[r] = self.ran[r].union(loader.getRange()[r])
        ...
        if self.corruption_mode != "Global":
            for r in self.headDict:
                ...
                if self.corruption_mode == "LCWA":
                    ...
                    headEntities = self.dom[r]
                elif self.corruption_mode == "LGlobal":
                    headEntities = self.ran[r]
                    ...
                    for ri in loaders[0].domDomCompatible[r]:
                        headEntities = headEntities.union(self.ran[ri])
                        #headEntities = headEntities.union(dom[ri])
                    for rj in loaders[0].domRanCompatible[r]:
                        headEntities = headEntities.union(self.dom[rj])
    ...
    def get_corrupted_scores(self, h, r, t, targets, type='head'):
        # Use only with LCWA, returns corruptions plus scores
        if type == "head":
            corrupted = set(self.headEntities[r]) - self.headDict[r][t]
            scores = np.array([targets['lcwa']] * len(corrupted))  # default lcwa
            scores[np.isin(list(corrupted), list(self.headEntities[-1]))] = targets['global']  # global
            scores[np.isin(list(corrupted), list(self.dom[r]))] = targets['tclcwa']  # tclcwa
        elif type == "tail":
            corrupted = set(self.tailEntities[r]) - self.tailDict[r][h]
            scores = np.array([targets['lcwa']] * len(corrupted))  # default lcwa
            scores[np.isin(list(corrupted), list(self.tailEntities[-1]))] = targets['global']  # global
            scores[np.isin(list(corrupted), list(self.ran[r]))] = targets['tclcwa']  # tclcwa
