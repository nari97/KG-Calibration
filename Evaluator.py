...
class Evaluator(object):
    ...
    def evaluate(self, model, materialize=False, accuracy=False):
        if self.use_gpu:
            model.cuda()
        ...
        for r in relations.keys():
            ...
            for t in relations[r]:
                ...
                rankhLess = torch.sum(positive > cHeads).item()
                ranktLess = torch.sum(positive > cTails).item()
                rankhEq += torch.sum(positive == cHeads).item()
                ranktEq += torch.sum(positive == cTails).item()
                ...
                if accuracy:
                    ...
                    fp += torch.sum(-cHeads > .5).item() + torch.sum(-cTails > .5).item()
                    tn += torch.sum(-cHeads <= .5).item() + torch.sum(-cTails <= .5).item()

                for i in range(1, totalTriples):
                    if materialize and scores[0] >= scores[i]:
                        self.add_triple(neg_triples, arrH[i], arrR[i], arrT[i], 1)

                ranks.append(self.frac_rank(rankhLess, rankhEq))
                ranks.append(self.frac_rank(ranktLess, ranktEq))
